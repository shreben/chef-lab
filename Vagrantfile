...
Vagrant.configure('2') do |config|
  ...
  chefsrv.vm.provision 'shell', inline: <<-SHELL
    ...
    if [ $? -ne 0 ]; then
      ...
      echo "192.0.0.30 apache" >> /etc/hosts
      echo "192.0.0.40 nginx" >> /etc/hosts
    fi
	  SHELL
  end
  ...
  config.vm.define 'apache' do |apache|
    ...
    apache.vm.network 'private_network', ip: '192.0.0.30'
  ...
  config.vm.define 'nginx' do |nginx|
    nginx.vm.hostname = 'apache'
    nginx.vm.network 'private_network', ip: '192.0.0.40'
